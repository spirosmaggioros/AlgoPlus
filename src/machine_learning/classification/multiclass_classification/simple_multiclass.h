    ...
    void verbose_acc_step_() {
        ...
        for (int i = 0; i < this->data_.size(); i++) {
            ...
            for (int c = 0; c < this->predictors_.size(); c++) {
                class_probs[c] = exp(std::inner_product(this->predictors_[c].begin(),
                                                        this->predictors_[c].end(),
                                                        this->data_[i].begin(),
                                                        0.0) + this->bias_);
                denom += class_probs[c];
            }
            ...
            int pred_class = std::distance(class_probs.begin(), 
...
public:
    ...
    explicit simple_multi_classification(
        ...
        this->predictors_ = std::vector<std::vector<double> >(num_classes, std::vector<double>(this->data_[0].size() - 1));
    ...
    inline void fit() {
        auto phi__ = [&](int class_, int j) -> double {
            double nom = std::exp(std::inner_product(
                this->predictors_[class_].begin(),
            ...
            for (size_t i = 0; i < this->predictors_.size(); i++) {
                denom += std::exp(std::inner_product(
                    this->predictors_[i].begin(),
        ...
        for (int epoch = 0; epoch < this->epochs_; epoch++) {
            std::vector<std::vector<double>> gradients(this->predictors_.size(), std::vector<double>(this->data_[0].size(), 0.0));
            ...
            for (int i = 0; i<this->predictors_.size(); i++) {
                for (int w = 0; w<this->data_[0].size(); w++) {
                    this->predictors_[i][w] -= this->learning_rate_ * gradients[i][w] / this->data_.size();
                }
            }
