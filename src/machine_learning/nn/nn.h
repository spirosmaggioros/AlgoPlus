...
namespace nn {
    ...
    /*
     * @brief Linear module. This implementation mostly follows PyTorch's implementation
     */
    class Linear {
    private:
...
inline void nn::Linear::update_weights(std::vector<double> const& input, double error, double learning_rate) {
    for (int i = 0; i<this->out_features_; i++) {
        for (int j = 0; j<this->in_features_; j++) {
